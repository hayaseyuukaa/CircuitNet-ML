...
# 脚本名称: run_gcn.sh
# 功能描述: 运行GCN模型进行拥塞预测任务
# 用法示例: 
#   ./run_gcn.sh train_n14    # 在CircuitNet 2.0 (N14)数据集上训练
#   ./run_gcn.sh train_n28    # 在CircuitNet 1.0 (N28)数据集上训练
#   ./run_gcn.sh train_ispd   # 在ISPD2015数据集上训练
#   ./run_gcn.sh test_n14     # 在CircuitNet 2.0 (N14)数据集上测试
#   ./run_gcn.sh test_n28     # 在CircuitNet 1.0 (N28)数据集上测试
#   ./run_gcn.sh test_ispd    # 在ISPD2015数据集上测试
...
# 确保使用bash激活conda
. "$HOME/anaconda3/etc/profile.d/conda.sh"
echo "调试: source命令执行状态: $?"

# 确认conda激活状态
conda info --envs
echo "调试: 当前活动环境: $(conda info --envs | grep '*' || echo '没有活动环境')"

# 取消当前环境并重新激活目标环境
conda deactivate 2>/dev/null
conda activate circuitnet
echo "调试: conda activate命令执行状态: $?"
...
if [ "$(conda info --envs | grep '*' | awk '{print $1}')" != "circuitnet" ]; then
    ...
    python --version
    # 列出已安装的关键包，以排除潜在问题
    pip list | grep -E 'torch|mmcv|mamba|networkx'
fi

# 根据参数执行相应操作
case "$1" in
    train_n14)
        WORK_DIR="$BASE_WORK_DIR/congestion_gcn"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/gcn_train_n14_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 2.0 (N14)数据集上训练GCN模型..."
...
EOF
        ...
        # 运行训练脚本并记录日志
        {
            ...
            echo "调试: 准备执行训练脚本..."
            echo "调试: 命令: python \"$SCRIPT_DIR/train.py\" --task congestion_gcn --save_path \"$WORK_DIR\" --arg_file \"$PARAMS_FILE\""
        ...
        ;;
    train_n28)
        WORK_DIR="$BASE_WORK_DIR/congestion_gcn_n28"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/gcn_train_n28_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 1.0 (N28)数据集上训练GCN模型..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
        ...
        ;;
    train_ispd)
        WORK_DIR="$BASE_WORK_DIR/congestion_gcn_ispd"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/gcn_train_ispd_${TIMESTAMP}.log"
        
        echo "开始在ISPD2015数据集上训练GCN模型..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_n14)
        WORK_DIR="$BASE_WORK_DIR/congestion_gcn"
        LOG_FILE="$LOG_DIR/gcn_test_n14_${TIMESTAMP}.log"
        
        echo "测试GCN模型（在CircuitNet 2.0 (N14)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_n28)
        WORK_DIR="$BASE_WORK_DIR/congestion_gcn_n28"
        LOG_FILE="$LOG_DIR/gcn_test_n28_${TIMESTAMP}.log"
        
        echo "测试GCN模型（在CircuitNet 1.0 (N28)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_ispd)
        WORK_DIR="$BASE_WORK_DIR/congestion_gcn_ispd"
        LOG_FILE="$LOG_DIR/gcn_test_ispd_${TIMESTAMP}.log"
        
        echo "测试GCN模型（在ISPD2015数据集上）..."
        echo "日志将保存到: $LOG_FILE"
