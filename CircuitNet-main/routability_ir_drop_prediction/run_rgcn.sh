...
# 脚本名称: run_rgcn.sh
# 功能描述: 运行RGCN模型进行拥塞预测任务
# 用法示例: 
#   ./run_rgcn.sh train_n14    # 在CircuitNet 2.0 (N14)数据集上训练
#   ./run_rgcn.sh train_n28    # 在CircuitNet 1.0 (N28)数据集上训练
#   ./run_rgcn.sh train_ispd   # 在ISPD2015数据集上训练
#   ./run_rgcn.sh test_n14     # 在CircuitNet 2.0 (N14)数据集上测试
#   ./run_rgcn.sh test_n28     # 在CircuitNet 1.0 (N28)数据集上测试
#   ./run_rgcn.sh test_ispd    # 在ISPD2015数据集上测试
...
case "$1" in
    train_n14)
        WORK_DIR="$BASE_WORK_DIR/congestion_rgcn"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/rgcn_train_n14_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 2.0 (N14)数据集上训练RGCN模型..."
...
# 运行训练脚本并记录日志
        {
            echo "===== CircuitNet 2.0 (N14)数据集RGCN训练日志 ====="
            echo "开始时间: $(date)"
            echo "模型: RGCNCongestion"
            echo "工作目录: $WORK_DIR"
        ...
        ;;
    train_n28)
        WORK_DIR="$BASE_WORK_DIR/congestion_rgcn_n28"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/rgcn_train_n28_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 1.0 (N28)数据集上训练RGCN模型..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行训练脚本并记录日志
        {
            echo "===== CircuitNet 1.0 (N28)数据集RGCN训练日志 ====="
            echo "开始时间: $(date)"
            echo "模型: RGCNCongestion"
            echo "工作目录: $WORK_DIR"
        ...
        ;;
    train_ispd)
        WORK_DIR="$BASE_WORK_DIR/congestion_rgcn_ispd"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/rgcn_train_ispd_${TIMESTAMP}.log"
        
        echo "开始在ISPD2015数据集上训练RGCN模型..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行统一的训练脚本并记录日志
    ...
    test_n14)
        WORK_DIR="$BASE_WORK_DIR/congestion_rgcn"
        LOG_FILE="$LOG_DIR/rgcn_test_n14_${TIMESTAMP}.log"
        
        echo "测试RGCN模型（在CircuitNet 2.0 (N14)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行测试脚本并记录日志
        {
            echo "===== CircuitNet 2.0 (N14)数据集RGCN测试日志 ====="
            echo "开始时间: $(date)"
            echo "模型: RGCNCongestion"
    ...
    test_n28)
        WORK_DIR="$BASE_WORK_DIR/congestion_rgcn_n28"
        LOG_FILE="$LOG_DIR/rgcn_test_n28_${TIMESTAMP}.log"
        
        echo "测试RGCN模型（在CircuitNet 1.0 (N28)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行测试脚本并记录日志
        {
            echo "===== CircuitNet 1.0 (N28)数据集RGCN测试日志 ====="
            echo "开始时间: $(date)"
            echo "模型: RGCNCongestion"
            echo "检查点: $LATEST_CHECKPOINT"
            echo "数据集路径: $N28_DATA_PATH"
    ...
    test_ispd)
        WORK_DIR="$BASE_WORK_DIR/congestion_rgcn_ispd"
        LOG_FILE="$LOG_DIR/rgcn_test_ispd_${TIMESTAMP}.log"
        
        echo "测试RGCN模型（在ISPD2015数据集上）..."
        echo "日志将保存到: $LOG_FILE"
