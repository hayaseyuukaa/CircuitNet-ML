...
# 脚本名称: run_swin_transformer.sh
# 功能描述: 运行Swin Transformer模型进行拥塞预测任务
# 用法示例: 
#   ./run_swin_transformer.sh train_n14    # 在CircuitNet 2.0 (N14)数据集上训练
#   ./run_swin_transformer.sh train_n28    # 在CircuitNet 1.0 (N28)数据集上训练
#   ./run_swin_transformer.sh train_ispd   # 在ISPD2015数据集上训练 (StepLR)
#   ./run_swin_transformer.sh train_ispd_cosine # 在ISPD2015数据集上训练 (CosineAnnealingLR)
#   ./run_swin_transformer.sh test_n14     # 在CircuitNet 2.0 (N14)数据集上测试
#   ./run_swin_transformer.sh test_n28     # 在CircuitNet 1.0 (N28)数据集上测试
#   ./run_swin_transformer.sh test_ispd    # 在ISPD2015数据集上测试
...
if [ $# -eq 0 ]; then
    echo "错误: 未提供参数"
    echo "用法: $0 [train_n14|train_n28|train_ispd|train_ispd_cosine|test_n14|test_n28|test_ispd]"
    exit 1
fi
...
case "$1" in
    train_n14)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/train_n14_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 2.0 (N14)数据集上训练Swin Transformer模型..."
...
# 运行训练脚本并记录日志
        ...
        ;;
    train_n28)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin_n28"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/train_n28_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 1.0 (N28)数据集上训练Swin Transformer模型..."
        echo "日志将保存到: $LOG_FILE"
        ...
        cat > "$PARAMS_FILE" << EOF
{
    ...
    "model_type": "SwinTransformerCongestion",
    "in_channels": 3,
    "out_channels": 1,
    "img_size": 256,
    "window_size": 8,
...
# 运行训练脚本并记录日志
        ...
        ;;
    train_ispd)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin_ispd"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/train_ispd_${TIMESTAMP}.log"
        
        echo "开始在ISPD2015数据集上训练Swin Transformer模型..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行统一的训练脚本并记录日志
        {
            echo "===== ISPD2015数据集Swin Transformer训练日志 ====="
            echo "开始时间: $(date)"
            echo "模型: Swin Transformer"
            echo "工作目录: $WORK_DIR"
            echo "数据集路径: $ISPD_DATA_PATH"
        ...
        ;;
    train_ispd_cosine)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin_ispd_cosine"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/train_ispd_cosine_${TIMESTAMP}.log"
        
        echo "开始在ISPD2015数据集上训练Swin Transformer模型 (CosineAnnealingLR)..."
        echo "日志将保存到: $LOG_FILE"
        ...
        # 运行新的ISPD2015训练脚本并记录日志
        {
            echo "===== ISPD2015数据集训练日志 (CosineAnnealingLR) ====="
            echo "开始时间: $(date)"
            echo "模型: Swin Transformer"
            echo "学习率调度器: CosineAnnealingLR"
            echo "工作目录: $WORK_DIR"
            echo "数据集路径: $ISPD_DATA_PATH"
            echo "====================================="
            echo ""
...
# 运行统一的训练脚本并记录日志
    ...
    test_n14)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin"
        LOG_FILE="${LOG_DIR}/test_n14_${TIMESTAMP}.log"
        
        echo "测试Swin Transformer模型（在CircuitNet 2.0 (N14)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行测试脚本并记录日志
    ...
    test_n28)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin_n28"
        LOG_FILE="${LOG_DIR}/test_n28_${TIMESTAMP}.log"
        
        echo "测试Swin Transformer模型（在CircuitNet 1.0 (N28)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
# 运行测试脚本并记录日志
    ...
    test_ispd)
        WORK_DIR="${BASE_WORK_DIR}/congestion_swin_ispd"
        LOG_FILE="${LOG_DIR}/test_ispd_${TIMESTAMP}.log"
        
        echo "测试Swin Transformer模型（在ISPD2015数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
        ...
        ;;
    # 为了兼容旧的命令行参数，保留旧的参数选项
    train)
        echo "提示: 旧的'train'命令已被重命名为'train_n14'"
        $0 train_n14
        ;;
    test)
        echo "提示: 旧的'test'命令已被重命名为'test_ispd'"
        $0 test_ispd
        ;;
    ispd2015)
        echo "提示: 旧的'ispd2015'命令已被重命名为'train_ispd'"
        $0 train_ispd
        ;;
    *)
        echo "错误: 未知参数 '$1'"
