...
# 脚本名称: run_routenet.sh
# 功能描述: 运行RouteNet模型进行拥塞预测任务
# 用法示例: 
#   ./run_routenet.sh train_n14    # 在CircuitNet 2.0 (N14)数据集上训练
#   ./run_routenet.sh train_n28    # 在CircuitNet 1.0 (N28)数据集上训练
#   ./run_routenet.sh train_ispd   # 在ISPD2015数据集上训练
#   ./run_routenet.sh test_n14     # 在CircuitNet 2.0 (N14)数据集上测试
#   ./run_routenet.sh test_n28     # 在CircuitNet 1.0 (N28)数据集上测试
#   ./run_routenet.sh test_ispd    # 在ISPD2015数据集上测试
...
case "$1" in
    train_n14)
        WORK_DIR="$BASE_WORK_DIR/congestion_routenet"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/routenet_train_n14_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 2.0 (N14)数据集上训练RouteNet模型..."
...
EOF
        ...
        ;;
    train_n28)
        WORK_DIR="$BASE_WORK_DIR/congestion_routenet_n28"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/routenet_train_n28_${TIMESTAMP}.log"
        ...
        echo "开始在CircuitNet 1.0 (N28)数据集上训练RouteNet模型..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
        ...
        ;;
    train_ispd)
        WORK_DIR="$BASE_WORK_DIR/congestion_routenet_ispd"
        mkdir -p "$WORK_DIR"
        LOG_FILE="$LOG_DIR/routenet_train_ispd_${TIMESTAMP}.log"
        
        echo "开始在ISPD2015数据集上训练RouteNet模型..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_n14)
        WORK_DIR="$BASE_WORK_DIR/congestion_routenet"
        LOG_FILE="$LOG_DIR/routenet_test_n14_${TIMESTAMP}.log"
        
        echo "测试RouteNet模型（在CircuitNet 2.0 (N14)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
        ...
        POSSIBLE_PATHS=(
            "$WORK_DIR"
            "$PROJECT_ROOT/work_dir/congestion_routenet"
            "/doc/gky/CircuitNet6/CircuitNet-main/work_dir/congestion_routenet"
            "/doc/gky/CircuitNet6/work_dir/congestion_routenet"
            "/media/user/6E55DE7F3C46090D2/gky/CircuitNet6/work_dir/congestion_routenet"
        )
        ...
        for path in "${POSSIBLE_PATHS[@]}"; do
            if [ -d "$path" ]; then
                echo "检查目录: $path"
                FOUND_CHECKPOINT=$(find "$path" -name "*.pth" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)
                
                if [ -n "$FOUND_CHECKPOINT" ]; then
                    LATEST_CHECKPOINT="$FOUND_CHECKPOINT"
                    echo "在 $path 找到检查点文件: $LATEST_CHECKPOINT"
                    break
                fi
            fi
        done
...
EOF
    ...
    test_n28)
        WORK_DIR="$BASE_WORK_DIR/congestion_routenet_n28"
        LOG_FILE="$LOG_DIR/routenet_test_n28_${TIMESTAMP}.log"
        
        echo "测试RouteNet模型（在CircuitNet 1.0 (N28)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
        ...
        POSSIBLE_PATHS=(
            "$WORK_DIR"
            "$PROJECT_ROOT/work_dir/congestion_routenet_n28"
            "/doc/gky/CircuitNet6/CircuitNet-main/work_dir/congestion_routenet_n28"
            "/doc/gky/CircuitNet6/work_dir/congestion_routenet_n28"
            "/media/user/6E55DE7F3C46090D2/gky/CircuitNet6/work_dir/congestion_routenet_n28"
        )
...
EOF
    ...
    test_ispd)
        WORK_DIR="$BASE_WORK_DIR/congestion_routenet_ispd"
        LOG_FILE="$LOG_DIR/routenet_test_ispd_${TIMESTAMP}.log"
        
        echo "测试RouteNet模型（在ISPD2015数据集上）..."
        echo "日志将保存到: $LOG_FILE"
        ...
        POSSIBLE_PATHS=(
            "$WORK_DIR"
            "$PROJECT_ROOT/work_dir/congestion_routenet_ispd"
            "/doc/gky/CircuitNet6/CircuitNet-main/work_dir/congestion_routenet_ispd"
            "/doc/gky/CircuitNet6/work_dir/congestion_routenet_ispd"
            "/media/user/6E55DE7F3C46090D2/gky/CircuitNet6/work_dir/congestion_routenet_ispd"
        )
        ...
        for path in "${POSSIBLE_PATHS[@]}"; do
            if [ -d "$path" ]; then
                ...
                if [ -f "$path/best_model.pth" ]; then
                    MODEL_PATH="$path/best_model.pth"
                    echo "在 $path 找到最佳模型: $MODEL_PATH"
                    break
                # 2. 然后检查是否有final_model.pth
                elif [ -f "$path/final_model.pth" ]; then
                    MODEL_PATH="$path/final_model.pth"
                    echo "在 $path 找到最终模型: $MODEL_PATH"
                    break
                # 3. 如果都没有，使用最新的检查点
                else
                    LATEST_CHECKPOINT=$(find "$path" -name "*.pth" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)
                    
                    if [ -n "$LATEST_CHECKPOINT" ]; then
                        MODEL_PATH="$LATEST_CHECKPOINT"
                        echo "在 $path 找到最新检查点: $MODEL_PATH"
                        break
                    fi
                fi
            fi
