...
# 脚本名称: run_gpdl.sh
# 功能描述: 运行GPDL模型进行拥塞预测任务
# 用法示例: 
#   ./run_gpdl.sh train_n14    # 在CircuitNet 2.0 (N14)数据集上训练
#   ./run_gpdl.sh train_n28    # 在CircuitNet 1.0 (N28)数据集上训练
#   ./run_gpdl.sh train_ispd   # 在ISPD2015数据集上训练
#   ./run_gpdl.sh test_n14     # 在CircuitNet 2.0 (N14)数据集上测试
#   ./run_gpdl.sh test_n28     # 在CircuitNet 1.0 (N28)数据集上测试
#   ./run_gpdl.sh test_ispd    # 在ISPD2015数据集上测试
...
case "$1" in
    train_n14)
        WORK_DIR="${BASE_WORK_DIR}/congestion_gpdl"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/gpdl_train_n14_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 2.0 (N14)数据集上训练GPDL模型..."
...
EOF
        ...
        ;;
    train_n28)
        WORK_DIR="${BASE_WORK_DIR}/congestion_gpdl_n28"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/gpdl_train_n28_${TIMESTAMP}.log"
        
        echo "开始在CircuitNet 1.0 (N28)数据集上训练GPDL模型..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
        ...
        ;;
    train_ispd)
        WORK_DIR="${BASE_WORK_DIR}/congestion_gpdl_ispd"
        mkdir -p "$WORK_DIR"
        LOG_FILE="${LOG_DIR}/gpdl_train_ispd_${TIMESTAMP}.log"
        
        echo "开始在ISPD2015数据集上训练GPDL模型..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_n14)
        WORK_DIR="${BASE_WORK_DIR}/congestion_gpdl"
        LOG_FILE="${LOG_DIR}/gpdl_test_n14_${TIMESTAMP}.log"
        
        echo "测试GPDL模型（在CircuitNet 2.0 (N14)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_n28)
        WORK_DIR="${BASE_WORK_DIR}/congestion_gpdl_n28"
        LOG_FILE="${LOG_DIR}/gpdl_test_n28_${TIMESTAMP}.log"
        
        echo "测试GPDL模型（在CircuitNet 1.0 (N28)数据集上）..."
        echo "日志将保存到: $LOG_FILE"
...
EOF
    ...
    test_ispd)
        WORK_DIR="${BASE_WORK_DIR}/congestion_gpdl_ispd"
        ALT_WORK_DIR="${BASE_WORK_DIR}/ispd2015_gpdl"  # 添加备选工作目录
        LOG_FILE="${LOG_DIR}/gpdl_test_ispd_${TIMESTAMP}.log"
        
        echo "测试GPDL模型（在ISPD2015数据集上）..."
        echo "日志将保存到: $LOG_FILE"
        ...
        # 1. 首先在主工作目录检查是否有best_model.pth
        if [ -f "$WORK_DIR/best_model.pth" ]; then
            MODEL_PATH="$WORK_DIR/best_model.pth"
            echo "使用主工作目录最佳模型: $MODEL_PATH"
        # 2. 然后在主工作目录检查是否有final_model.pth
        elif [ -f "$WORK_DIR/final_model.pth" ]; then
            MODEL_PATH="$WORK_DIR/final_model.pth"
            echo "使用主工作目录最终模型: $MODEL_PATH"
        # 3. 在备选工作目录检查是否有best_model.pth
        elif [ -f "$ALT_WORK_DIR/best_model.pth" ]; then
            MODEL_PATH="$ALT_WORK_DIR/best_model.pth"
            echo "使用备选工作目录最佳模型: $MODEL_PATH"
        # 4. 在备选工作目录检查是否有final_model.pth
        elif [ -f "$ALT_WORK_DIR/final_model.pth" ]; then
            MODEL_PATH="$ALT_WORK_DIR/final_model.pth"
            echo "使用备选工作目录最终模型: $MODEL_PATH"
        # 5. 如果主工作目录有检查点，使用最新的检查点
        elif [ -d "$WORK_DIR" ] && [ "$(find "$WORK_DIR" -name "*.pth" -type f | wc -l)" -gt 0 ]; then
            LATEST_CHECKPOINT=$(find "$WORK_DIR" -name "*.pth" -type f -printf "%T@ %p\n" | sort -n | tail -1 | cut -d' ' -f2-)
            MODEL_PATH="$LATEST_CHECKPOINT"
            echo "使用主工作目录最新检查点: $MODEL_PATH"
        # 6. 如果备选工作目录有检查点，使用最新的检查点
        elif [ -d "$ALT_WORK_DIR" ] && [ "$(find "$ALT_WORK_DIR" -name "*.pth" -type f | wc -l)" -gt 0 ]; then
            LATEST_CHECKPOINT=$(find "$ALT_WORK_DIR" -name "*.pth" -type f -printf "%T@ %p\n" | sort -n | tail -1 | cut -d' ' -f2-)
            MODEL_PATH="$LATEST_CHECKPOINT"
            echo "使用备选工作目录最新检查点: $MODEL_PATH"
        # 7. 如果都没有找到模型文件
        else
            echo "错误: 在 $WORK_DIR 和 $ALT_WORK_DIR 中均未找到模型文件，请先训练模型"
            exit 1
        fi
